 ...
 alter table  mkordtmpfs  ADD COLUMN shelfnos_id_to_pare numeric(22,0)  DEFAULT 0  not null;

 alter table mkordtmpfs  ADD COLUMN starttime timestamp(6);

 alter table  mkordtmpfs  ADD COLUMN locas_id_pare numeric(38,0)  DEFAULT 0  not null;
...
mkordtmpf.shelfnos_id_to_pare   mkordtmpf_shelfno_id_to_pare,
mkordtmpf.starttime  mkordtmpf_starttime,
mkordtmpf.locas_id_pare   mkordtmpf_loca_id_pare
 from mkordtmpfs   mkordtmpf,
  r_persons  person_upd ,  r_itms  itm ,  r_locas  loca ,  r_prjnos  prjno ,  r_itms  itm_pare ,  r_shelfnos  shelfno_to ,  r_mkprdpurords  mkprdpurord ,  r_shelfnos  shelfno_to_pare ,  r_locas  loca_pare 
  where       mkordtmpf.persons_id_upd = person_upd.id      and mkordtmpf.itms_id = itm.id      and mkordtmpf.locas_id = loca.id      and mkordtmpf.prjnos_id = prjno.id      and mkordtmpf.itms_id_pare = itm_pare.id      and mkordtmpf.shelfnos_id_to = shelfno_to.id      and mkordtmpf.mkprdpurords_id = mkprdpurord.id      and mkordtmpf.shelfnos_id_to_pare = shelfno_to_pare.id      and mkordtmpf.locas_id_pare = loca_pare.id     ;
...
,mkordtmpf_shelfno_id_to_pare  numeric (22,0)
,mkordtmpf_starttime   timestamp(6) 
...
,mkordtmpf_loca_id_pare  numeric (38,0)
...
,itm_unit_id  numeric (22,0)
 ...
 ALTER TABLE mkordtmpfs ADD CONSTRAINT mkordtmpf_shelfnos_id_to_pare FOREIGN KEY (shelfnos_id_to_pare)
																		 REFERENCES shelfnos (id);
 ALTER TABLE mkordtmpfs ADD CONSTRAINT mkordtmpf_locas_id_pare FOREIGN KEY (locas_id_pare)
																		 REFERENCES locas (id);
