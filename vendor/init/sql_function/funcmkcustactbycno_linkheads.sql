drop function  if exists funcmkcustactbycno_linkheads;
CREATE OR REPLACE FUNCTION funcmkcustactbycno_linkheads(loca_code_cust text,cno text)
...
select  
 ...
 inner join (select sum(qty_src) qty ,tblid from linkcusts link 
  ...
  where       custord.custord_cno = $2 and custord.loca_code_cust  = $1
