...
CREATE OR REPLACE FUNCTION public.func_get_free_ord_stk_v1(induedate character varying, inprjnos_id numeric, initms_id numeric, inprocessseq numeric,inshelfnos_id numeric)
 RETURNS TABLE(tblname character varying, tblid numeric, priority text, due numeric, starttime timestamp without time zone, duedate timestamp without time zone, 
 				processseq numeric, mlevel numeric, itms_id numeric, shelfnos_id numeric, prjnos_id numeric, trngantts_id numeric, alloctbls_id numeric, qty numeric,
...
BEGIN
	RETURN QUERY
	 EXECUTE 'select   ---  free　を求めるsql
						...
						inner join shelfnos s on s.id = gantt.shelfnos_id_to
